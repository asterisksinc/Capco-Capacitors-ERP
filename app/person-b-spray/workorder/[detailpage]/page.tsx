...
export default function SprayDetailPage({ params }: DetailPageProps) {
    ...
    if (activeTab === "Winding") {
      ...
      payload.forEach((item) => {
        addFlowRow(orderId, "Spray", {
          ...
          sprayType: item.sprayType,
          feedRate: item.feedRate,
          pressureSitting: item.pressureSitting,
          stage: "Spray Done",
          timestamp: dateTime,
          status: "Completed"
        });
          ...
          <div key={`spray-step1-${idx}`} className="rounded-[12px] border border-[#DDE1E8] p-4 bg-white">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
              <div className="flex flex-col gap-2">
                ...
                <ScannerInput
                  value={row.spId}
                  onChange={(e) => updateSprayRow(idx, { spId: e.target.value })}
                  onScanData={(data) => updateSprayRow(idx, { spId: data })}
              ...
              <div className="flex flex-col gap-2">
                ...
                <select value={row.sprayType} onChange={(e) => updateSprayRow(idx, { sprayType: e.target.value })} className="h-[42px] rounded-[8px] border border-[#DDE1E8] px-3 text-[14px]">
                  <option value="Zinc-spray">Zinc-spray</option>
                  <option value="Copper-spray">Copper-spray</option>
                </select>
              </div>
              <div className="flex flex-col gap-2">
                <label className="text-[13px] font-medium text-[#171717]">Feed Rate</label>
                <input value={row.feedRate} onChange={(e) => updateSprayRow(idx, { feedRate: e.target.value })} placeholder="Feed rate" className="h-[42px] rounded-[8px] border border-[#DDE1E8] px-3 text-[14px]" />
              </div>
              <div className="flex flex-col gap-2">
                <label className="text-[13px] font-medium text-[#171717]">Pressure Setting</label>
                <input value={row.pressureSitting} onChange={(e) => updateSprayRow(idx, { pressureSitting: e.target.value })} placeholder="Pressure setting" className="h-[42px] rounded-[8px] border border-[#DDE1E8] px-3 text-[14px]" />
              </div>
            </div>
          </div>
        ))}
  ...
  const renderReviewCards = () => {
    ...
    return sprayRowsInput.map((item, idx) => (
      <div key={`spray-${idx}`} className="rounded-[12px] border border-[#78CFFA] bg-[#F4FBFF] p-4 grid grid-cols-1 md:grid-cols-2 gap-y-2 gap-x-6 text-[14px] text-[#49526A]">
        ...
        <p>Feed Rate: {item.feedRate}</p>
        <p>Pressure Setting: {item.pressureSitting}</p>
      </div>
    ));
  };
      ...
      <section className="bg-white border-b border-[#EBEBEB] hidden md:block">
        <div className="px-6 py-5 flex items-center justify-between">
          <div className="flex items-center gap-2 text-[14px]">
            <Link href="/person-b-spray/workorder" className="text-[#5C5C5C] hover:text-black">Work Orders</Link>
                    ...
                    {currentConfig.columns.map((col) => {
                      if (String(col.key) === "options") {
                        ...
                        const qrDetails: Record<string, string> = isWD
                          ...
                          : { "SP ID": (row as any).spId ?? "", "Linked WD ID": (row as any).linkedWdId ?? "", "Spray Type": (row as any).sprayType ?? "", "Feed Rate": (row as any).feedRate ?? "", "Status": (row as any).status ?? "" };
                        return (
